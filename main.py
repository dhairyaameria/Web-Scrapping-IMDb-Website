...
url = 'https://m.imdb.com/what-to-watch/popular/?ref_=watch_wls_tab'
movie_name = []
movie_rate = []
description = []
creators = []
...
def write_csv():
    ...
    data = list((itertools.zip_longest(*file_list)))
    # list(*(zip_longest(file_list)))
    with open('imdb.csv', mode='w',newline='') as file_object:
        wr = csv.writer(file_object)
        wr.writerow(['Movie name', 'Movie rate', 'Description', 'Creators', 'Trailer link', 'More Info'])
        wr.writerows(data)
    print('SUCCESS: Check file to see the RESULT!')

def main():
    page = requests.get(url)
    soup = BeautifulSoup(page.content, 'lxml')
    movies = soup.findAll('div', {'class':'ipc-poster-card ipc-poster-card--baseAlt ipc-poster-card--dynamic-width TitleCard-sc-1e5jqmp-0 egRGeD has-action-icons ipc-sub-grid-item ipc-sub-grid-item--span-2'})
    # print(movies)
    for movie in movies:
